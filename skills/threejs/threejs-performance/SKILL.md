...
description: >
  ...
  culling, texture size, lights/shadows, work in the animation loop.
  Do not use for React re-renders or resource dispose-only leaks.
---
...
1. **Measure** `renderer.info.render.calls` and FPS if possible.
...
4. **Textures:** power-of-two, compressed (KTX2) when pipeline exists; don’t ship 4k everywhere.
5. **Lights/shadows:** few real-time lights; tight shadow cameras; bake if static.
6. **LOD / frustum culling** for large scenes.
7. **Pixel ratio** cap (especially mobile).
